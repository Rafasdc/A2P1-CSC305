...
const char * fshader_square = " \
        ...
        void main() {\
            ...
            if (texture(tex,UV) == vec4(0,0,0,0)){\
            \
            color = vec4(0,0,0,0);\
            } else {\
        ...
        void main() { \
           float noise = 10.0 * -.10 * turbulence(.5 * octa_vpoints);\
           float b = 5.0 * pnoise( 0.05 * octa_vpoints, vec3( 100.0 ) );\
           float displacement = -10.0f *noise+ b;\
           vec3 octa_vpointss = octa_vpoints+displacement;\
           gl_Position =  pr*view*model*vec4(octa_vpoints.x,octa_vpoints.y+0.25,octa_vpoints.z,1.0f);\
           Normal = octa_vpoints;\
